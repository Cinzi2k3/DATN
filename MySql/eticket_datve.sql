...
CREATE TABLE `datve` (
  ...
  `soluongve` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`madatve`),
  KEY `rưr_idx` (`matau`),
  KEY `ưrrr_idx` (`matoa`),
  CONSTRAINT `rưr` FOREIGN KEY (`matau`) REFERENCES `tau` (`matau`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `ưrrr` FOREIGN KEY (`matoa`) REFERENCES `toa` (`matoa`) ON DELETE CASCADE ON UPDATE CASCADE
